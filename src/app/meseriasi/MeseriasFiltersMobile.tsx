...
import {
    countActiveMeseriasFilters,
    DEFAULT_MESERIAS_FILTERS,
    type MeseriasFiltersState,
} from "@/lib/meserias";
import type { Meserias } from "@/types/meserias";
import { SlidersHorizontal, X } from "lucide-react";
import { MeseriasFiltersForm } from "./MeseriasFiltersForm";

type MeseriasFiltersMobileProps = {
    filters: MeseriasFiltersState;
    meseriasi: Meserias[];
    onApply: (filters: MeseriasFiltersState) => void;
};
...
export function MeseriasFiltersMobile({
    filters,
    meseriasi,
    onApply,
}: MeseriasFiltersMobileProps) {
    const [mounted, setMounted] = useState(false);
    const [open, setOpen] = useState(false);
    const [closing, setClosing] = useState(false);
    const [entered, setEntered] = useState(false);
    const [draft, setDraft] = useState(filters);
    const activeCount = countActiveMeseriasFilters(filters);
    ...
    const handleReset = () => {
        setDraft(DEFAULT_MESERIAS_FILTERS);
    };
    ...
    return (
        <>
            <button
                type="button"
                aria-label="Filtrează meseriași"
...
export default MeseriasFiltersMobile;
